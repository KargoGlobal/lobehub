...
/**
 * Image models that operate purely on an input image and have no prompt.
 *
 * `prompt` is mandatory on every model card (see ModelParamsMetaSchema below),
 * so these models still declare one — but the UI hides the prompt box and the
 * create flow substitutes a label instead of rejecting the empty value. Without
 * this, `createImage` throws "prompt is empty" and the model is unusable.
 */
export const PROMPTLESS_IMAGE_MODELS = new Set<string>(['fal-ai/birefnet/v2']);

export const isPromptlessImageModel = (modelId: string) => PROMPTLESS_IMAGE_MODELS.has(modelId);
